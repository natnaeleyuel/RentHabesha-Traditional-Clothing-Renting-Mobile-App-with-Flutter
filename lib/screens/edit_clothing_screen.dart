...
class EditClothingScreen extends StatefulWidget {
  ...
  const EditClothingScreen({required this.clothingId, Key? key}) : super(key: key);
...
class _EditClothingScreenState extends State<EditClothingScreen> {
  ...
  void initState() {
    super.initState();
    debugPrint('Editing clothing with ID: ${widget.clothingId}');
    context.read<ClothingBloc>().add(LoadClothingForEdit(widget.clothingId));
  }

  void _initializeData(ClothingItem clothing) {
    debugPrint('Initializing with title: ${clothing.title}');
    debugPrint('Existing images: ${clothing.images}');
    debugPrint('Initializing with type: ${clothing.type}');
    debugPrint('Initializing with rental duration: ${clothing.rentalDuration}');
    debugPrint('Initializing with price: ${clothing.price}');
    debugPrint('Initializing with description: ${clothing.description}');
    debugPrint('Initializing with care instruction: ${clothing.careInstruction}');

    _nameController.clear();
    _descriptionController.clear();
    _priceController.clear();
    _careInstructionController.clear();
    _newImages.clear();
    _existingImageUrls.clear();

    _nameController.text = clothing.title;
    _descriptionController.text = clothing.description ?? '';
    _priceController.text = clothing.price;
    _careInstructionController.text = clothing.careInstruction ?? '';
    _selectedType = clothing.type;
    _selectedRentalPeriod = clothing.rentalDuration;
    _existingImageUrls = List.from(clothing.images);
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (pickedFiles != null && pickedFiles.isNotEmpty) {
        ...
        setState(() {
          _newImages.addAll(validFiles.whereType<File>());
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ClothingBloc, ClothingState>(
      listener: (context, state) {
        if (state is ClothingLoadedForEdit){
          _initializeData(state.clothing);
        }
      ...
      builder: (context, state) {
        return Scaffold(
          appBar: CustomAppBar(title: 'Edit Clothing', onBackPressed: () => context.go(AppRoute.home.path),),
          body: _isLoading
          ...
          itemBuilder: (context, index) {
            ...
            return Stack(
              children: [
                isExisting
                    ? Image.network(
                  '${Globals.baseUrl}${_existingImageUrls[index]}',
                  fit: BoxFit.cover,
